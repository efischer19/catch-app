...
issues to achieve WCAG 2.1 Level AA compliance. Integrate automated
accessibility testing into CI, perform manual screen reader testing, and verify
keyboard navigation across all views.

## Acceptance Criteria

- [ ] An automated accessibility testing tool (e.g., `axe-core` via `@axe-core/playwright` or `jest-axe`) is integrated into the CI pipeline
...
- [ ] Form controls (team selector) have associated labels
...
- [ ] Reduced motion preference (`prefers-reduced-motion`) is respected for any animations or transitions
- [ ] An accessibility statement page documents compliance level and any known limitations

## Implementation Notes

**♿ Accessibility Coordinator notes — this is the central accessibility ticket:**

- Use `axe-core` (via `@axe-core/playwright` or a Vitest integration) for
  automated testing. It catches approximately 57% of WCAG issues
  automatically — the rest require manual verification.
- Manual testing is required for issues that automated tools can't catch:
  reading order, focus management, meaningful screen reader announcements.
- **Screen reader test script:** Create a manual test checklist and store it
  in the repo (e.g., `docs-src/manual-a11y-checklist.md`):
...
- Accessibility and performance are complementary, not conflicting. Semantic
  HTML is faster to parse, proper headings improve SEO, and keyboard
  navigation requires no additional JavaScript overhead.
...
- Track accessibility regressions: if a new component is added without proper
  accessibility attributes, the CI should fail.
