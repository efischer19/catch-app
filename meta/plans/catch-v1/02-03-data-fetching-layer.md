...
- [ ] A `DataService` module provides typed functions: `getTeamSchedule(teamId)`, `getUpcomingGames()`
- [ ] Each function returns data typed with the generated TypeScript interfaces (e.g., `Promise<GoldTeamSchedule>`)
- [ ] The base URL for the Gold CDN is configurable via environment variable (for dev/staging/prod)
- [ ] HTTP errors (4xx, 5xx) are caught and surfaced as typed error result objects, not thrown exceptions
...
- Use the browser's native `fetch` API — no axios or other HTTP libraries
  needed. This keeps the bundle minimal.
...
- Consider using a stale-while-revalidate pattern: show cached data
...
- Loading states must be communicated to screen readers via `aria-live`
  regions or `role="status"` announcements. The data layer should expose
  loading/error/success states that UI components can use for accessible
  status updates.
- Error states should display meaningful messages, not raw HTTP status codes.

**🔧 Data Pipeline Janitor notes:**

- The data layer should be completely isolated from UI components. Components
  receive data via function calls — they never call `fetch` directly. This
  separation enables easy testing and future refactoring.
- If the CDN returns a 404 (e.g., a team file doesn't exist yet because the
  pipeline hasn't run), surface a user-friendly "Data not yet available"
  message rather than crashing.

**⚾ Baseball Edge-Case Hunter notes:**

- During the off-season, Gold JSON files may be stale or empty. The data
  layer should handle this gracefully — an empty schedule is not an error.
