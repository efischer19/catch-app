...
- [ ] A video player component accepts a `condensed_game_url` string and renders a `<video>` element
...
- [ ] If the video URL is invalid or fails to load, a user-friendly error message is displayed (not a broken video element)
- [ ] The player includes a "Cast" button placeholder that opens the Google Cast flow (implemented in ticket 04-02)
...
- [ ] Keyboard controls work: Space to play/pause, arrow keys to seek, Escape to close the modal
- [ ] Video playback does not auto-start (user must click play to begin loading the video)
...
- The `.mp4` URLs come from MLB's CDN. These are typically 720p or 1080p
  condensed game recaps, 5-15 minutes long, 50-200 MB in size.
- **Cross-origin concerns:** The video URLs are on MLB's domain, not ours.
  CORS may block `<video>` playback in some browsers. Test with actual MLB
  URLs early. If blocked, the frontend may need to offer a "Watch on MLB.com"
  link instead of embedding — document this risk.
...
- Test on: Chrome, Firefox, Safari (desktop and mobile), and Chrome on
  Android TV (for Cast-adjacent scenarios).

**♿ Accessibility Coordinator notes:**

- The `<video>` element should have `aria-label` describing the content
  (e.g., "Condensed game: Yankees vs Red Sox, July 10").
- Native `<video>` controls are generally accessible out of the box. Do not
  build custom controls unless necessary — custom controls often break
  accessibility.
...
- Do NOT preload video content. The `.mp4` files are large and expensive.
  Only begin loading when the user explicitly clicks play.
- The service worker (ticket 02-04) must NOT cache these videos.
