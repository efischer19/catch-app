...
consolidated list of all MLB games happening today and in the near future.
This view fetches `gold/upcoming_games.json` from the CDN and renders games
grouped by date, showing matchups, start times, and scores for completed games.

## Acceptance Criteria

- [ ] The today's slate view is the app's default landing page at `/`
- [ ] The view loads and displays data from `gold/upcoming_games.json` via the DataService
- [ ] Games are grouped by date with clear date headers (e.g., "Thursday, July 10, 2025")
- [ ] Today's games are visually emphasized (highlighted section, distinct styling, or "Today" badge)
- [ ] Each game shows: away team @ home team, start time or final score, and game status
...
- **Time zones:** The Gold data stores UTC timestamps. The frontend must
  ...
  `Intl.DateTimeFormat` for localization — no timezone library needed.
...
- The game list should be navigable by keyboard: Tab to move between games,
  Enter to view boxscore or watch video.
- Today's section should be announced by screen readers: consider using
  `aria-current="date"` on today's heading.
- Timezone handling: display the timezone abbreviation (e.g., "7:10 PM ET")
  so users know which timezone is shown.
