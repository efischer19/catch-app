...
- [ ] The service worker does NOT cache `.mp4` video URLs (these are large and hosted on MLB's CDN)
...
- Document the caching strategy in an ADR: which resources are precached,
  ...
  performance and UX decision that future contributors need to understand.

**⚡ PWA Performance Fanatic notes:**

- Use Workbox (Google's service worker library) for reliable caching
  strategies. It is well-tested and handles edge cases (cache quotas, update
  flows, etc.). Vite has a `vite-plugin-pwa` that integrates Workbox
  automatically.
...
- When the app transitions from cached to fresh data, do not cause a jarring
  page reload. Update the content in-place and announce the update via an
  `aria-live` region (e.g., "Schedule updated").
...
- Service worker caching reduces CDN requests. Users who check scores daily
  ...
  not once per page load. This keeps bandwidth costs near zero.
