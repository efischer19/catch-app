...
This document outlines the ethical principles that govern all data gathering, scraping, and API interaction conducted by the `catch-app` project. Our goal is to build useful software while acting as good citizens of the digital world. Adherence to these principles is mandatory for all code, whether generated by AI or written by humans. Our project's long-term viability depends on the trust and respect we show to the data sources we rely on.

`catch-app` displays baseball statistics sourced from the MLB Stats API via the [catch-data](https://github.com/efischer19/catch-data) companion repository, which performs the actual API interactions. This document applies to both repositories.
...
* **Example `User-Agent`:** `catch-app/0.1 (+https://github.com/efischer19/catch-app)`
